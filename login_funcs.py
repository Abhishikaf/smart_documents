...
def create_table(c):
    c.execute("CREATE TABLE IF NOT EXISTS usertable(usertype TEXT, username TEXT, password TEXT, licence TEXT, wallet_address TEXT)")

def add_data(conn, usertype, username, password,licence,wallet_address):
    c = conn.cursor()
    c.execute("INSERT INTO usertable(usertype, username, password,licence,wallet_address) VALUES (?, ?, ?, ?, ?)",(usertype, username, password,licence, wallet_address))
    conn.commit()

def login_user(c, usertype,username,password,licence,wallet_address):
    if usertype == "Notary":
...
def sign_up(conn, new_usertype): 
    c=conn.cursor()
    st.write("Please create a new account to be able use our services")
...
def sign_in(conn, usertype):
    c=conn.cursor()
    st.write("Please sign in to use the Smart Contract services")
